...
**CRON FREEZE (2 Jul 2026, owner decision), REVISED 10 Jul 2026: the LIFECYCLE crons are back on; everything else stays DISABLED** (`active=false`; schedules preserved in `cron.job`) — the app is pre-release, nobody uses it, and cron-driven retry loops were behind the €45 Google Places bill. Consequences while frozen: no ingestion (content ages), no Saturday digest, no enrichment — all fine with zero users; the embeddings corpus was fully backfilled *before* the freeze so the concierge matches the whole catalog. Any function can still be invoked by hand.

**Lifecycle revision (10 Jul 2026, owner-approved — events are worthless after they happen; venues are stable for years):**
- **Re-enabled (all zero-cost — SQL or free edge invocations, no LLM, no metered APIs):** `archive-stale-daily` 04:00 · `reset-tonight` 00:05 · `rotate-tonight-daily` 04:05 · `wa-dedup-picks` 04:30 · `wa-purge-archived` 04:45 · `wa-ingest-osm` **monthly** (1st, 03:30 — was weekly; it exists only to keep `last_seen_at` honest).
- **Purge retention 90 → 14 days** (`wa_purge_old_archived`, migration `purge_retention_14_days`): soft-archive is a 2-digest-cycle grace for Saved's gone-detection + mistake reversal, then hard DELETE (`pick_embeddings` cascade-clean). Backlog purge on 10 Jul removed 2,858 dead events; a hand-run of archive-stale the same day archived the 467 that had expired during the freeze.
- **`archive-stale` v9:** venue OSM-absence window 21 → 90 days (21d false-flags every venue between monthly OSM pings; the freeze proved it — flags reset 10 Jul).
- **Re-enable everything at launch:** `select cron.alter_job(jobid, active => true) from cron.job;` — then restore the three dialled-down schedules: `wa-process-staging`→`*/30 * * * *`, `embed-picks-auto`→`*/30 * * * *`, `wa-geocode-picks`→`20 * * * *` via `select cron.alter_job(jobid, schedule => '<old>') from cron.job where jobname = '<name>';` (they were left at the P0 reduced cadence: `12 * * * *` / `40 6,12,18,23 * * *` / `20 5 * * *`).
- **What each cron family called while live** (all free-tier/open since the Places retirement — the freeze stops token waste and noise, not spend, because nothing metered remains): `ingest-*` → Telegram web, RSS, Fienta JSON, Helsinki Linked Events, venue websites, OSM Overpass · `wa-process-staging` → **Groq tokens** on new staging rows · `embed-picks-auto` → **Gemini free-tier embedding tokens** · `send-digest` → **Gemini + Resend email** · `draft-column`/`generate-context`/`classify-moods` → **Groq/Gemini tokens** · `geocode-picks`/`enrich-venues` → Nominatim/Wikidata (rate-limited free) · lifecycle crons (`archive-stale`, `rotate-tonight`, `reset-tonight`, `wa-dedup-picks`, `wa-purge-*`, `wa-reconcile-absent`, `wa-ingest-health`, `cleanup-match-cache`) → SQL only, no external calls.
- Spend caps remain owner-console-only (not settable via API): Groq console spend limit, Google Cloud budget alert ≤€5 — set + note here when done.
...
- **Gemini text fallback retired 2 Jul 2026:** `pipeline_config.gemini_fallback_enabled=false` — every text-generation function now checks this flag before ever calling Gemini. Next fallback lane per the new policy = OpenRouter `:free` (live, see below).
- `process-staging` (v41/v44) → Groq primary → OpenRouter `:free` → Gemini gated by the flag above. Holds the `CITY_CONTEXT` map.
- `generate-context` (v13/v16) → same 3-tier chain. Only picks with `context_md IS NULL`. **Audit fix (3 Jul):** the Gemini call was deployed ungated (fired unconditionally as a last resort, contradicting the retirement flag) — found during a post-hoc audit and gated to match process-staging; redeployed.
- `draft-column` (v16/v19) → same 3-tier chain. Weekly 140-word column. **Same ungated-Gemini bug found + fixed** in the same audit pass.
- `classify-moods` (v2, 3 Jul 2026) → **found completely undocumented and Gemini-only** (hard `503` if `GEMINI_API_KEY` was ever absent) — the one pipeline function that never got the Groq-first policy. Brought onto the same 3-tier chain (Groq → OpenRouter → gated Gemini); prompt/response reshaped to `{"results":[...]}` so Groq/OpenRouter's `json_object` mode (which requires a top-level object, not a bare array) validates. Verified live: classified the 2 remaining untagged picks via Groq, 0 unclassified remain.
- `send-digest` (v11) → Gemini `2.5-flash` (low volume) with a static-copy fallback if the key is absent (`if (!GEMINI) return fallback;` — already graceful, left as-is). Composes the "your saved events changed" block from `digest_opt_ins.user_id` + `pick_changes`.
- `match-pick` (v10) → ranking on Groq only (`find_many`, topK=5); embeddings on Cloudflare Workers AI `@cf/baai/bge-m3` (see below).
...
- **`calendar-feed` v1 (2 Jul 2026):** public GET, serves `text/calendar` per city (`?city=`) and per curator (`&handle=`) from active dated non-pending picks; date semantics mirror `share.js`'s client ICS builder; 12h TTL hints; linked from Today's This Week header + curator pages. The market-scan "put the curator in your calendar" primitive — no push, no cron.
- **OpenRouter `:free` fallback lane — LIVE (3 Jul 2026):** owner created `OPENROUTER_API_KEY` + set it in Supabase secrets. `process-staging` (v41/v44), `generate-context` (v12/v15), `draft-column` (v15/v18) deployed with a `callOpenRouter` fallback between Groq and the retired Gemini path. Model pinned via `OPENROUTER_MODEL` — probed live and set to `openai/gpt-oss-120b:free` (the originally-planned `meta-llama/llama-3.3-70b-instruct:free` 429'd on the shared free pool at probe time; gpt-oss-120b answered clean). Verified: `generate-context` real invocation (`{"limit":1}`) → `processed:1`; `draft-column` real invocation (`{"city":"riga"}`) → drafted a column via `provider:"groq"` (OpenRouter is reachable but untested end-to-end since Groq hasn't failed yet — expected, it's a fallback).
- **Google fully exited (3 Jul 2026):** owner deleted the Google Cloud billing account and the Places API key. Embeddings — the last remaining Google call — moved to **Cloudflare Workers AI `@cf/baai/bge-m3`** (1024-dim, free 10k neurons/day). `pick_embeddings.embedding` migrated `vector(768)`→`vector(1024)` (HNSW index rebuilt); `embed-picks` v4 rewritten for Workers AI + batches 25 texts/call; `match-pick` v10 query-embeds the same way. Full re-embed backfilled same-day (all 4 cities drained to 0 missing). **Bug found + fixed in the same pass:** `match-pick`'s `PICK_COLS` still selected `world_x`/`world_y`, columns dropped when the illustrated map was retired — every hybrid-search pick fetch silently 400'd, so the concierge always returned empty even though retrieval itself worked. Fixed by dropping those columns from the select (response shape kept `world_x`/`world_y: null` for backward compat). Verified live: hybrid queries in Tallinn and Riga both return relevant, real hits.
- **All 30 crons re-confirmed disabled** (see the freeze note above) — the OpenRouter/Workers-AI work above was validated via direct function invocation, not cron traffic.
- **Post-hoc audit (3 Jul 2026) — repo/production drift + gating gaps found and closed:** (1) the `pick_embeddings → vector(1024)` migration was applied via MCP but never written to `supabase/migrations/` — journal copy added retroactively (`20260703_embeddings_bge_m3_1024.sql`); (2) `generate-context`/`draft-column` had the Gemini-retirement gate written to the repo file but the *deployed* function was one version behind and still called Gemini ungated — redeployed with the fix, confirmed via `get_edge_function`; (3) `classify-moods` (above) was Gemini-only with no documented status at all. Also checked and ruled out: no other function references the dropped `world_x`/`world_y` columns; no CHECK constraints reject the new `coords_source='venue_join'` / `venue_details.source='overture_index'` values; `places_index`/`pick_embeddings` RLS unaffected by the type migration; the one-time `load-places-index` token was never committed to git (only the 410 stub was); `enrich-pick-images` already no-ops gracefully on the now-deleted `GOOGLE_PLACES_API_KEY`.
- **Provider revision — applied, not proposal.** The free-tier AI + places/events strategy in `docs/provider-strategy-jul26.md` is now fully executed: Overture-based `discover-venues`, OpenRouter fallback lane, Workers AI embeddings, cron freeze. Remaining open items there are optional hardening (Groq prompt caching, hybrid-retrieval *ranking* quality), not blockers.
...
- **Pick photos moved off Google's CDN → Supabase Storage (19 Jul 2026):** `enrich-pick-images` had stored Google's *ephemeral* resolved `lh3.googleusercontent.com/place-photos/…` URIs directly on `picks.image_url`; Google expires them (measured **22 of 32 distinct URLs already 403**). Fix: nulled the 25 dead picks (→ kind-glyph fallback) and copied the 14 survivors (10 distinct photos) into the new **public `pick-images` storage bucket** via one-shot fn **`import-pick-photos`** (verify_jwt:false; `dry_run` defaults **true**; object name = `sha256(url)[:16].<ext>`, `x-upsert`; repoints only rows still on a googleusercontent URL, so it's idempotent — now a no-op and safe to delete). End state: `picks.image_url` = 0 google / 14 storage / 33 null. `proxifyImage` passes storage URLs through untouched; CSP `img-src https:` already covers the storage host; the visual/aria suites keep blocking `*.supabase.co/**/storage/**` for determinism (no baseline change). New picks still get photos from `enrich-images` (Wikidata/Wikimedia); a future re-photograph of the nulled venues would want the same fetch→storage step baked into that path.

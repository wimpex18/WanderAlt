...
- **OpenRouter `:free` fallback lane (repo-ahead, 2 Jul 2026):** `process-staging` (v41 in repo) + `generate-context` + `draft-column` now carry a `callOpenRouter` fallback between Groq and the retired Gemini path — **inert until `OPENROUTER_API_KEY` exists** (model pinned via `OPENROUTER_MODEL`, default `meta-llama/llama-3.3-70b-instruct:free`). NOT yet deployed (crons frozen = no traffic to validate against); when the owner adds the secret, deploy these three via MCP preserving each `verify_jwt`, then hand-invoke once each.

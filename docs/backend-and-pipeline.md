...
**CRON FREEZE (2 Jul 2026, owner decision): ALL 30 cron jobs are DISABLED** (`active=false`; schedules preserved in `cron.job`) — the app is pre-release, nobody uses it, and cron-driven retry loops were behind the €45 Google Places bill. Nothing scheduled runs until re-enabled. Consequences while frozen: no ingestion (content ages), no Saturday digest, no lifecycle rotation (tonight/archival), no enrichment — all fine with zero users; the embeddings corpus was fully backfilled *before* the freeze so the concierge matches the whole catalog. Any function can still be invoked by hand.

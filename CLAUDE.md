...
- Current per-function status:
  ...
  - `send-digest` (**v11**) → Gemini `2.5-flash` (weekly email intro, no grounding; v11 fixed the nonexistent `3.5-flash` that had every intro silently falling back to static copy). Low volume; not yet moved to Groq-primary (candidate if ever needed). v11 also composes the per-recipient "your saved events changed" block for account-linked recipients (`digest_opt_ins.user_id` + `pick_changes` journal).

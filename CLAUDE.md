...
- **Event rows are photo-forward cards (June 2026):** Discover **Events** rows (`renderRow`) are `.list-row--card` — a duotone `.thumb--lg` venue photo (left, reusing the app's standard photo treatment) + title/meta/quote body, falling back to the initials tile when a pick has no `image_url`. `content-visibility` keeps them lazy. **Places** rows stay social-glyph-only (OSM venues carry no photos), so the photo card is Events-only.

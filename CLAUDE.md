...
| `taste.js` | Taste-profile onboarding (energy/company/money axes); exposes `WA.taste.matchParams()` + `WA.taste.tasteScore(entry)`. **Three surfaces are gently re-ordered by `tasteScore`** (stable sort — ties keep curation/recency; 0 score = no reorder) once a taste profile exists: Today's **This Week**, Discover's default **Relevance** sort (Events only; skipped on active query / Places), and Saved's **Reading** list (undated saves; Going stays soonest-first). Each carries one subtle "· tuned to you" cue (section-sub on Today; results count on Discover; Reading seg-note) — on Discover/Saved the cue is a `.taste-cue` link to `index.html#taste-onboarding` so the reader can adjust (briefing.js re-opens the taste check from that hash even when already onboarded). On-device only (localStorage); curation stays primary — no per-card "for you" badges. |
...
| `view-transition.js` | Cross-document **card→hero** View-Transition helper (on index/discover/saved). Tags the clicked card photo (or Tonight hero) with `view-transition-name: venue-hero` so it morphs into venue.html's `.detail-hero`. Reduced-motion-skipped; no-op without VT support. |
...
- **Plugins & skills in cloud sessions** are driven by the committed `.claude/settings.json` (the interactive `/plugin` command is local-CLI-only and unavailable on the web). Cloud installs `enabledPlugins` from their marketplace at session start (needs network to the source). Currently enabled: `design@knowledge-work-plugins` (Anthropic's Design plugin — `/design-critique`, `/design-system`, `/design-handoff`, `/accessibility-review`, `/user-research`, `/research-synthesis`), `frontend-design@claude-plugins-official` (Anthropic's official frontend/design skill), and `ui-ux-pro-max@ui-ux-pro-max-skill` (third-party UI/UX design-intelligence skill). Non-official marketplaces are declared in `extraKnownMarketplaces`. Plugins run arbitrary code — vet before adding. **Project skill** `.claude/skills/wanderalt-design/SKILL.md` seeds the brand/design system so those plugins produce on-brand output. Full reference: `docs/skills-and-plugins.md`.

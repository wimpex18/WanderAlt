...
- **Type:** Fraunces (`--ff-display`, titles + curator quote), Inter (`--ff-body`), Geist Mono (`--ff-mono`, uppercase labels). Self-hosted woff2, no Google CDN. **On any screen that surfaces a curator pick (Today, venue, curator) the curator quote is the largest element; browse/filter pages (Discover, Saved) have no quote and lead with the `.page-head` title.**

...
**Walks is a two-week test, not a feature** (6b). Three hand-written routes in `walks.json`, one card on Explore, and the number that decides its future is **third-stop opens** — ≥25% build the generator, <10% delete the lot. Every stop was chosen because its `opening_hours` is filed: hours are missing on half the Tallinn catalogue, and 6b's own risk flag is that a walk-in-progress screen without them "lies to people outdoors — worse than not shipping". A generator would pick blind; a human curating around the gap is what makes the promise honest. If the number comes back low, delete `walks.json`, `walk.*`, the `.wa-walk*` rules and the Explore card — it is built to be deleted.
...
- **`hours.js` models public holidays, because 24 venues file `PH off`.** The two easy answers are both wrong: rejecting the clause blanked a museum's whole week 365 days a year to avoid being wrong on fourteen, and stripping it prints `→18` over a door locked for Jaanipäev. So the four countries' holidays are a table in `hours.js` — fixed dates plus the feasts computed off Easter, keyed by `WA.CITY`, which is why `isHoliday()` reads the active city rather than taking a parameter. PH is a genuine eighth bucket (`week.ph`), and `null` (no clause, inherit the weekday — OSM's own default) must stay distinguishable from `[]` (shut on holidays). It overrides in **both** directions: a bar filing `PH,Su 12:00-23:00` is open on Christmas Day and shut on an ordinary Wednesday. Month and year selectors (`Aug-May:`, `Jul,Aug Mo-Tu …`, `We[-1] off`) are still refused on purpose — those genuinely change the answer by season, and guessing one is the lie the whole file is written to avoid.
...
- **The offline claim is backed now** (6f#5). `sw.js` precaches the shell and keeps the last picks/venues responses, stamped with `x-wa-cached-at`, so 6d's stronger copy is true: saves are here, a list renders, and the banner prints *how stale* it is. Navigations are network-first — nothing is content-hashed here, so a cache-first HTML rule would serve yesterday's page. `/sw.js` is `no-cache` in `_headers`; a stale worker pins the whole app. Never cache a signed-in response.

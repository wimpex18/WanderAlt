...
Front-end: city plate SVG (`assets/vilnius-overview.svg`), city.js entry (`status: 'live'`), static venue seed in `catalog.js` (offline fallback). The city is now **selectable** in the dropdown so the team can dogfood it. This is an internal-testing unlock, **not a public launch** — Vilnius has **0 curated picks and no curator voice yet**, so Discover's **Places** mode is populated (~410 OSM venues) while **Events and Today render graceful empty states** (the Tonight hero shows a "curators are warming up" line; This Week shows the city-plate empty card). The backend pipeline keeps ingesting, so the DB fills with real content over time. Revert to `status: 'coming'` if the empty Events surface needs hiding before a public launch.
...
- `ingest-ra` **v1** deployed — pulls Vilnius electronic events from RA's GraphQL (`https://ra.co/graphql`, area **561**, browser-like headers; HTML frontend is Cloudflare-gated but the API answers from a datacenter IP). Validated once: **24 events** staged (Paviljonas, Utopija, Sodas 2123, Vasaros Terasa, …). `process-staging` turns them into Vilnius picks on its normal tick.

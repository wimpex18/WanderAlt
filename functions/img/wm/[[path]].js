/* ============================================================
   WanderAlt — Wikimedia image proxy (Cloudflare Pages Function)
   ------------------------------------------------------------
   Route:  /img/wm/<url-encoded-target>, deployed with the site.
   Job:    Re-fetch a Wikimedia image, strip the Set-Cookie headers
           Wikimedia attaches, and serve the bytes through Cloudflare's
           edge cache (24 h), so no third-party cookie reaches the reader.

   Client: supabase.js rewrites Wikimedia URLs to /img/wm/… on any
           non-localhost host. The local dev server has no Functions, so
           URLs pass through unchanged there.
   ============================================================ */
...
/* Raster types only. Commons accepts user-uploaded SVG, and an
   image/svg+xml response served from our /img/wm/… is ACTIVE

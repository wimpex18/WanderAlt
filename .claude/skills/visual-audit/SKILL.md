...
`.list-row` carries `content-visibility: auto` (perf: Discover/Saved hold 100+ rows). Rows that were never near the viewport are **skipped by the renderer**, so naive fullPage captures show them as **empty hairline boxes** (and `innerText` probes return `""`). Real users scrolling see everything — artifact, not a bug (the July 2026 audit chased this ghost across three pages before pinning it). `audit.js` now scrolls through the page before capturing to force paint; any ad-hoc puppeteer shot of a long list must do the same, and any "rows render empty" finding must be re-verified after an in-page scroll before it is reported.

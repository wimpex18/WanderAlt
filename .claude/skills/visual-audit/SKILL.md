...
1. **Capture.** `npm run audit` (Playwright, Jul 2026 — verify/e2e stay Puppeteer) → every public page **including auto-derived venue/curator/place param pages** at **390 / 768 / 1440**, each scroll-hydrated first, then captured as **viewport-sized segments** `.screenshots/audit/<page>-<width>-sNN.png` (s01 = top of page, last segment = true bottom). It also prints a per-page numeric census: distinct icon (svg) sizes + any horizontal overflow + any segment-cap truncation. Segments beat fullPage twice over: each PNG is exactly one real user viewport (no artifacts), and each reads back at full legibility instead of a squashed 8000px strip. For a *specific state* (filter applied, dialog open, signed-in), take a targeted shot with a quick Playwright snippet at the same widths.
...
## Reading the segments right
Segments are **viewport-truth** — the two historical fullPage artifact classes are designed out, so don't re-learn their reflexes:
- Fixed chrome (topbar, docked bottom bar) **correctly appears in every segment**. Repetition across segments is truth, not duplication — never "fix" it.
- Rows are scroll-hydrated before capture, so an **empty row/region is now a real finding**, not a lazy-render artifact (the Jul 2026 audit twice nearly filed artifacts as bugs — that's what forced this redesign).
- Map segments: **markers/clusters are truth; the vector basemap is not** — it does not rasterise in headless on this machine (probed Jul 2026: tiles fetch, swiftshader and metal GL contexts both create, canvas never paints). A blank basemap behind correctly-placed markers is an environment artifact — verify tiles on the Cloudflare PR preview; missing/misplaced markers ARE a real finding.
- A page longer than the segment cap logs a `⚠ … not captured` line — check the tail on a live page rather than assuming the report covered it.
If Playwright's browser is missing ("Executable doesn't exist"), run `npx playwright install chromium`.

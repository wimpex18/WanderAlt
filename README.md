...
Every page is a plain `.html` file at the repo root with a matching `.js` renderer, all sharing one `wa.css` — 49 tokens, 13 components, two themes, ~2,480 lines, replacing a 9,118-line `styles.css`. Pages: Explore (`index.html`), Tonight (`discover.html`), Saved, detail (`detail.html`), source (`source.html`), profile, admin, about, 404. The filenames of the first two are deliberately unchanged so every shared link in the wild still resolves. `_redirects` maps the retired `venue.html`, `place.html`, `curator.html`, `map.html` and `search.html` onto their replacements.

Explore is a browsing surface. One capsule answers Where / When / What, and four scope tabs narrow it to All / Tonight / Places / Walks — icon over label, underline on the active one. The scope tabs sit under the capsule at every width, aligned to its 840px column so the two read as one control group. They were briefly Explore's desktop masthead, with the app nav dropped there — that shipped and was refuted: with nothing saved yet the saved strip is absent by design, so a first-time desktop visitor had **no route to Saved at all**, and two four-item icon rows both containing "Tonight" meant a scope filter on one page and a whole page on the other eight. Below sit named carousel sections, 6-up at 1280, each carrying a count in the subtitle so a thin section reads as a short row rather than a hole, and each ending in a "See all N" cell that bridges to the timetable. The weekly Thursday email sits at the foot of the page rather than interrupting the list. The masthead shortcut to it was removed: one masthead does one job, and the email is reachable from You and About.

Tonight is the deciding surface: a dense timetable where each row leads with its rail — time, then walking distance. The four facet buttons collapsed into the same capsule plus one filter sheet, and every toggle in that sheet prints its consequence ("9 of tonight's 19 are free") while the primary key states the outcome ("Show 5 gigs"). Map is a mode, and a mode is never empty: it always carries a way out, a "search this area" that re-queries the viewport, and a drawer listing the picks in view, so tapping a pin stays optional. In map mode the capsule chips and the Filters key stick under the top bar below 1024, because a mode you cannot re-filter without scrolling out of it is a mode with its controls taken away. Pins carry time and distance rather than a price — and time alone when location is unknown, rather than inventing one. Pin and row highlight together, and the selected pin never clusters, so that pairing always holds. Pins re-project on every frame of a drag; the drawer under them rebuilds only when the drag ends and only if its contents actually changed, because rewriting a list of names per frame costs ten times as much and throws away focus and scroll while someone is reading it. Every camera move honours `prefers-reduced-motion` — the destination is identical, it just arrives without the tween. Overlapping pins collapse into a petrol count bubble, since a pin here is a ~90px label and a hundred of them at city zoom is a pile rather than a map; the count is a fact, so it is mono and petrol, never lime. The map states its own coverage gap, because a partial pin count next to a full list reads as a map failure instead of the data gap it is.
...
**The pipeline runs again (Aug 2026).** All 32 jobs are active: every ingest, `wa-process-staging` hourly, the enrichment set (including `wa-enrich-venue-images` nightly and `wa-verify-images` weekly), the lifecycle housekeeping, and the Thursday digest.
...
`send-digest-thursday` (was `send-digest-saturday`, retimed 13 Aug 2026 to `0 7 * * THU` = 10:00 Tallinn) sends the weekly email; it was turned on 5 Aug 2026 once `send-digest` was deployed (see below). Every job is active. The last inactive one, `draft-column-weekly`, was unscheduled in

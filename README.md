...
Every page is a plain `.html` file at the repo root with a matching `.js` renderer, all sharing one `wa.css` — 48 tokens, 13 components, two themes, ~2,480 lines, replacing a 9,118-line `styles.css`. Pages: Explore (`index.html`), Tonight (`discover.html`), Saved, detail (`detail.html`), source (`source.html`), profile, admin, about, 404. The filenames of the first two are deliberately unchanged so every shared link in the wild still resolves. `_redirects` maps the retired `venue.html`, `place.html`, `curator.html`, `map.html` and `search.html` onto their replacements.

...
### Progress tracker (updated per PR — last: PR #65 fourth batch, June 2026)
...
| F-4 Saved header/empty canon | **FIXED** — `.picks-empty` cards per segment, H1 → "Saved" | F-15 filter-rail/sort placement | **FIXED** — option (b) shipped: facet dropdown-pill row above results, sort right-aligned in the row; V-11: count + first row now in viewport at 1280×900 (pin lands one short scroll down — remaining lever is chrome height, tracked under F-16) |
| F-5 Profile CTA rule | **FIXED** — `.profile-actions` stacks full-width <768 | F-16 Today rhythm pass | **partial** — F-6 lede stack done; remaining: chrome-height compression above Discover's panes (the last V-11 pin lever), label-scale audit |
...
| F-11 duplicate thumbs | **FIXED** — 91 legacy non-spatial photos nulled in Supabase (7 active) + consecutive-dup render guard on Today/Discover lists | F-22 control-weight inversion | **mostly fixed** via F-15b (workhorse filters now one labelled row); search-field scale still pending under F-16 |

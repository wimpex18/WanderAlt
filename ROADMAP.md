...
*WWDC 2026 context (June 8–9; iOS 27 dev beta 1 — researched via MacRumors' iOS-27/WWDC roundups + 9to5mac's iOS-27 guide on June 11):* a "Snow Leopard"-style refinement year — performance and cleanup over visual overhaul. The design facts that matter to us: **Liquid Glass was tuned for readability** (sharper, more detailed icon rendering; a user-facing **translucency-level slider**; more **depth/separation** in nav bars, menus and buttons); search and assistant merged into one **"Search or Ask"** surface (Siri lives in the Dynamic Island, replacing Spotlight); **extra-large widgets** (Calendar/Photos/Music); app-level redesigns (Music artist pages, Screen Time) and a changed Notification-Center access pattern that promptly ate "broke 15 years of muscle memory" coverage; iPadOS 27/macOS 27 get design parity. Net read for WanderAlt: Apple is *refining glass for legibility, not expanding it* — our three-surface restraint is the current pattern, and the items below follow the same grain. (Web quota capped mid-research; icon news in these sources = sharper rendering within Liquid Glass, not a reshape — re-verify against the iOS 27 HIG diff when quota resets.)

- **`prefers-reduced-transparency` on the three glass surfaces** *(new — direct iOS 27 echo, shippable now).* iOS 27 hands users a system translucency slider; the web's equivalent signal is the `prefers-reduced-transparency` media query. When set, swap `--glass-bg`/`--glass-blur` for the existing solid fallback surfaces (we already have the `@supports` path — this adds the user-preference path). Small, accessibility-grade, on-brand.
- **Chrome separation micro-pass** *(new — iOS 27 echo, restrained).* iOS 27 adds depth/separation to chrome for readability over content. Our audit points: the glass topbar/nav hairline over photo-heavy scrolls, and the facet popover's edge over busy lists. Remedy vocabulary stays canon: hairline weight/`--c-paper-deep` deepening only — no new shadows or gradients.
...
- **Icon ladder: sharpness audit + layered master** *(updated by WWDC; also the native-app on-ramp).* iOS 27 renders icons with more detail/sharpness inside Liquid Glass. Actions: (1) audit the Beacon favicon/PWA SVGs at 16–64px for how the lime diamond holds detail; (2) produce a **layered icon master** in `brand/` — petrol tile and lime diamond as separate layers — so one source feeds Apple's layered/Liquid-Glass icon pipeline AND Android adaptive icons when native shells happen. Brand-kit work only, no app change.

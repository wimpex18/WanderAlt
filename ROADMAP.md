...
Still-open product ideas (unranked, kept for the record): curator weekly synthesis ("Reading lately" on `curator.html`, the strongest editorial idea on the shelf — the current `columns` table backs this up: 15 of 16 rows ever written are stuck in `draft`, only 1 ever `published` (Tallinn, 6 May 2026), so Today's `.week__rail` column slot has been empty on all but one week since launch; the fix is moving the concept to curator.html rather than trying to keep a weekly-cron column pipeline fed), chrono-pin map, lineage edges, time-traveled briefing.

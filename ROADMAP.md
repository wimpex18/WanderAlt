...
**v0.7.12.1 hotfix (Jul 2026) — SHIPPED.** Two independent fixes, no frame/layout changes: (1) the four `assets/<city>-overview.svg` city-plate illustrations were re-cropped — `viewBox` widened 20% (+180 each side, background rect extended to match) so `object-fit: cover` renders the skyline pulled back within the *same* `.city-banner` frame (88px mobile / 120px desktop, untouched) — buildings read smaller with visible headroom + ground, instead of a tight, cropped close-up; verified via before/after screenshots at 390/1440 across all 4 cities. (2) `ingest-hanzas-perons`, `ingest-echo-gone-wrong`, `ingest-splendidpalace`, `ingest-telliskivi`, `ingest-kinobize`, `ingest-ra` were all missing `?on_conflict=channel,message_id` on their `staging_messages` POST (see `docs/backend-and-pipeline.md` → Gotchas) — confirmed live via `api` logs showing raw 409s (100% error rate on `hanzasperons`); fixed + redeployed + reverified all 6 with `errored:0`.

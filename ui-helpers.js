...
(() => {
  ...
  /* External web / social links for a venue or place, rendered as a compact
     ...
     buttons. Glyphs are the standard monochrome brand marks (Simple Icons
     Facebook + Instagram, a globe for the website); they inherit currentColor
     and go petrol on hover, so the two-tone brand holds (no brand colours).
     Pass { name, website, facebook, instagram }; only the links present are
     rendered, and '' comes back when there are none. Single source of truth
     for the social glyphs on detail pages. */
  const SOCIAL_SVG = {
